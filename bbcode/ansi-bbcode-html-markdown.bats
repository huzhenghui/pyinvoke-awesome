...
@test "convert 1m to b to strong" {
    ...
    [ "$result" == "convert 1m **to b to** strong\\" ]
}
...
@test "convert 3m to i to em" {
    ...
    [ "$result" == "convert 3m *to i to* em\\" ]
}

@test "convert 4m to u to strong" {
    result="$(echo -e "convert 4m \033[4mto u to\033[0m strong" | ansifilter --bbcode | inv bbcode-parser-format | pandoc --from=html --to=markdown)"
    [ "$result" == "convert 4m **to u to** strong\\" ]
}